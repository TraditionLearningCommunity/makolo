...
class EventListView(ListView):
    ...
    paginate_by = 20
    def get_queryset(self): return get_events_visible_to(self.request.user)
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["can_create_event"] = user_can_manage_events(self.request.user)
        context["has_organization"] = bool(self.request.user.is_authenticated and OrganizationMembership.objects.filter(user=self.request.user, is_active=True).exists())
        return context
...
class EventCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def test_func(self): return user_can_manage_events(self.request.user)
    def handle_no_permission(self):
        if self.request.user.is_authenticated: raise PermissionDenied("Créez une organisation ou rejoignez une équipe ayant le droit de gérer les événements.")
        return super().handle_no_permission()
    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated and user_can_manage_events(request.user) and not OrganizationMembership.objects.filter(user=request.user, is_active=True).exists(): ensure_personal_organization(request.user)
    ...
    def form_valid(self, form):
        ...
        return response
    def get_success_url(self): return reverse("events:detail", kwargs={"slug": self.object.slug})
...
class EventDetailView(DetailView):
    ...
    slug_field = "slug"
    slug_url_kwarg = "slug"
    def get_queryset(self): return get_events_visible_to(self.request.user, for_detail=True)
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
...
class EventUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    slug_url_kwarg = "slug"
    login_url = "core:login"
    def get_queryset(self): return get_manageable_events(self.request.user)
    def test_func(self): return user_can_manage_event(self.request.user, self.get_object())
    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs(); kwargs["user"] = self.request.user; return kwargs
    def form_valid(self, form):
        with transaction.atomic():
...
class EventTransitionView(LoginRequiredMixin, View):
    ...
    def post(self, request, slug):
        ...
        try: self.service(event=event, actor=request.user)
        except (ValidationError, PermissionDenied) as exc: messages.error(request, "; ".join(getattr(exc, "messages", [str(exc)])))
        else: messages.success(request, self.success_message)
        return redirect("events:detail", slug=event.slug)

class EventPublishView(EventTransitionView):
    service = staticmethod(publish_event); success_message = "Événement publié."
class EventCancelView(EventTransitionView):
    service = staticmethod(cancel_event); success_message = "Événement annulé."
class EventCompleteView(EventTransitionView):
    service = staticmethod(complete_event); success_message = "Événement marqué comme terminé."
