...
class SpaceConsoleAuthorityTests(TestCase):
    ...
    def test_scanner_mandate_uses_canonical_access_and_duplicate_refusal(self):
        ...
        url = reverse("organizations:console-control-activity-scan", kwargs={"slug": self.space_a.slug, "activity_id": self.activity_a.pk})
        first = self.client.post(url, {"token": token, "occurrence": str(self.occurrence.pk)})
        self.assertEqual(first.status_code, 200)
        self.assertTrue(first.json()["accepted"])
        self.assertEqual(first.json()["result"], AccessUseResult.ACCEPTED)
        ...
        self.assertEqual(second.json()["result"], AccessUseResult.ALREADY_USED)
