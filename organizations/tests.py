...
class OrganizationPermissionTests(TestCase):
    ...
    def test_creator_gets_default_team_membership_owner_mandate_and_legacy_projection(self):
        ...
        legacy = OrganizationMembership.objects.get(
            organization=self.organization,
            user=self.owner,
        )
    ...
    def test_space_admin_cannot_grant_space_ownership(self):
        admin = User.objects.create_user(
            ...
            email="space-admin@makolo.test",
            password="StrongPass2026!",
        )
        target = User.objects.create_user(
            username="owner-target",
            email="owner-target@makolo.test",
            password="StrongPass2026!",
        )
        add_or_update_member(
            organization=self.organization,
    ...
    def test_owner_can_leave_after_second_owner_exists(self):
        second_owner = User.objects.create_user(
            ...
            email="second-owner@makolo.test",
            password="StrongPass2026!",
        )
        add_or_update_member(
            organization=self.organization,
    ...
    def test_organization_workspace_is_available_to_authorized_team_member(self):
        ...
        self.assertContains(response, "Responsable activité")
    ...
    def test_team_member_without_mandate_cannot_open_workspace(self):
    ...
    def test_cannot_manage_team_of_another_space(self):
        other_owner = User.objects.create_user(
            ...
            email="other-owner@makolo.test",
            password="StrongPass2026!",
        )
        other = create_organization(creator=other_owner, name="Other Space")
        self.client.force_login(self.owner)
        response = self.client.get(f"/organizations/{other.slug}/team/add/")
        self.assertEqual(response.status_code, 403)

    def test_web_team_invitation_creates_team_membership_and_mandate(self):
        target = User.objects.create_user(
            username="invited-finance",
            email="invited-finance@makolo.test",
            password="StrongPass2026!",
        )
        self.client.force_login(self.owner)
        response = self.client.post(
            f"/organizations/{self.organization.slug}/team/add/",
