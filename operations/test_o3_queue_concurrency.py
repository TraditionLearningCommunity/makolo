...
@skipUnless(connection.vendor == "postgresql", "O3 concurrency requires PostgreSQL row locking.")
class O3QueueConcurrencyTests(TransactionTestCase):
    ...
    def test_simultaneous_entries_receive_distinct_monotonic_sequences(self):
        barrier = Barrier(2)
        ...
        self.assertEqual(list(QueueEntry.objects.filter(queue=self.queue).order_by("sequence").values_list("sequence", flat=True)), [1, 2])
