...
from .selectors import active_space_places
from .services import create_place_for_space, deactivate_space_place, update_space_place


class SpacePlaceListView(LoginRequiredMixin, View):
    template_name = "geography/space_place_list.html"

    def get(self, request, slug):
        organization = get_object_or_404(Organization, slug=slug)
        if not can(request.user, PermissionCode.SPACE_PLACES_VIEW, organization):
            raise PermissionDenied("Vous ne pouvez pas voir les Lieux de cet Espace.")
        can_manage = can(request.user, PermissionCode.SPACE_PLACES_MANAGE, organization)
        relations = (
            SpacePlace.objects.filter(organization=organization).select_related("place")
            if can_manage
            else active_space_places(organization)
        )
        return render(
            request,
            self.template_name,
            {"organization": organization, "relations": relations, "can_manage": can_manage},
        )
...
class SpacePlaceCreateView(LoginRequiredMixin, View):
    ...
    def get(self, request, slug):
        ...
        return render(
            ...
            self.template_name,
            {
                ...
                "relation_form": SpacePlaceForm(prefix="relation", initial={"is_active": True}),
                "creating": True,
            },
        )
    ...
    def post(self, request, slug):
        ...
        if place_form.is_valid() and relation_form.is_valid():
            create_place_for_space(
                actor=request.user,
                organization=organization,
                place_data=place_form.cleaned_data,
                relation_data=relation_form.cleaned_data,
            )
            messages.success(request, "Lieu ajouté à l’Espace.")
            return redirect("organizations:places", slug=organization.slug)
        return render(
            request,
            self.template_name,
            {"organization": organization, "place_form": place_form, "relation_form": relation_form, "creating": True},
            status=400,
        )
...
class SpacePlaceUpdateView(LoginRequiredMixin, View):
    ...
    def _relation(self, request, slug, pk):
        relation = get_object_or_404(
            SpacePlace.objects.select_related("organization", "place"),
            pk=pk,
    ...
    def get(self, request, slug, pk):
        ...
        return render(
            ...
            self.template_name,
            {
                ...
                "relation": relation,
                "relation_form": SpacePlaceForm(instance=relation, prefix="relation"),
                "creating": False,
            },
        )
    ...
    def post(self, request, slug, pk):
        ...
        if form.is_valid():
            ...
            return redirect("organizations:places", slug=relation.organization.slug)
        return render(
            request,
            self.template_name,
            {"organization": relation.organization, "relation": relation, "relation_form": form, "creating": False},
            status=400,
        )
...
class SpacePlaceDeactivateView(LoginRequiredMixin, View):
    def post(self, request, slug, pk):
        relation = get_object_or_404(
            SpacePlace.objects.select_related("organization"),
            pk=pk,
            organization__slug=slug,
        )
