...
from .models import (
    PolicyStatus, RecognitionAllocation, RecognitionObjectEvaluation, RecognitionPolicy,
    RecognitionRule, RecognitionSignal,
)
...
def _resolve_shares(contributors):
    ...
    for contributor in contributors:
        ...
        if not subject_id or subject_type not in {"profile", "space"}:
            continue
        try:
            strength = Decimal(str(contributor.get("weight", contributor.get("share", 1))))
        except Exception:
            continue
        if strength <= 0:
            continue
        ...
        if subject_type == "profile":
            ...
            subject = Organization.objects.filter(pk=subject_id).first()
        if subject is None:
            continue
...
def process_signal(*, signal, window, policy):
    rules = list(policy.rules.filter(enabled=True, signal_kind=signal.signal_kind).order_by("priority", "code"))
    ...
    for rule in rules:
        ...
        for grant in result.grants:
            key = f"profile:{grant.account.profile_id}" if grant.account.profile_id else f"space:{grant.account.space_id}"
            ...
            RecognitionAllocation.objects.get_or_create(
                ...
                defaults={
                    ...
                    "share": detail.share.share if detail else (Decimal(grant.points) / Decimal(max(result.receipt.pool_points, 1))),
                    "points": grant.points,
                    "evidence": {"signal_id": signal.signal_id, "outcome_identity": signal.outcome_identity},
                },
            )
...
def run_default_recognition_cycle(*, now=None, max_windows=7):
    ...
    for _ in range(max(int(max_windows), 1)):
        ...
        for signal in signal_provider(window.starts_at, window.ends_at):
            total_slices += process_signal(signal=signal, window=window, policy=policy)
