...
def _previous_rule_states(*, object_type, object_id, policy_version):
    previous = (
        RecognitionObjectEvaluation.objects.filter(
            object_type=object_type, object_id=object_id, window__policy_version=policy_version,
        ).order_by("-created_at").values_list("explanation", flat=True).first()
    )
...
def _has_resolvable_contributor(rule, evaluated):
    ...
    for contributor in evaluated.attribution_shares:
        ...
        if strategy == "profile_only" and subject_type != "profile": continue
        if strategy == "space_only" and subject_type != "space": continue
...
def _resolve_object_shares(rows, total_utility):
    if total_utility <= 0: return tuple()
    subject_utility = defaultdict(lambda: Decimal("0")); subject_modes = defaultdict(lambda: defaultdict(lambda: Decimal("0"))); subjects = {}
    allowed_modes = {choice for choice, _ in CausalMode.choices}
    for rule, evaluated, effective_utility, _state in rows:
        if effective_utility <= 0: continue
        attribution = rule.attribution or {}; strategy = attribution.get("strategy", "signal_contributors")
        if strategy == "unattributed": continue
        ...
        for contributor in evaluated.attribution_shares:
            ...
            if subject_type not in {"profile", "space"} or not subject_id: continue
            if strategy == "profile_only" and subject_type != "profile": continue
            if strategy == "space_only" and subject_type != "space": continue
            weight = max(Decimal("0"), decimal_value(contributor.get("weight", 1)))
            subject = _resolve_subject(subject_type, subject_id)
            if weight <= 0 or subject is None: continue
            mode = str(contributor.get("causal_mode", CausalMode.OPERATE))
            if mode not in allowed_modes: mode = CausalMode.OPERATE
            key = f"{subject_type}:{subject.pk}"; subjects[key] = (subject_type, subject); contributors.append((key, mode, weight))
        total_weight = sum((weight for _key, _mode, weight in contributors), Decimal("0"))
        if total_weight <= 0 or fraction <= 0: continue
        attributable = effective_utility * fraction
        for key, mode, weight in contributors:
            amount = attributable * weight / total_weight; subject_utility[key] += amount; subject_modes[key][mode] += amount
    resolved = []
    for key in sorted(subject_utility):
        amount = subject_utility[key]
        if amount <= 0: continue
        subject_type, subject = subjects[key]; dominant_mode = max(subject_modes[key].items(), key=lambda row: (row[1], row[0]))[0]
        resolved.append(ResolvedShare(
            share=AttributionShare(share=min(Decimal("1"), amount / total_utility), profile=subject if subject_type == "profile" else None, space=subject if subject_type == "space" else None, description="Valeur rendue possible dans Makolo"),
            causal_mode=dominant_mode, strength=amount,
        ))
...
def process_signal_group(*, signals, window, policy):
    ...
    object_type = signals[0].object_type; object_id = signals[0].object_id
    ...
    parameters = policy.parameters or {}
    semantic_window_hours = max(1, min(int(parameters.get("semantic_window_hours", policy.standard_window_hours) or policy.standard_window_hours), 8760))
    rules = list(policy.rules.filter(enabled=True, signal_kind__in={signal.signal_kind for signal in signals}).order_by("priority", "code"))
    group_identity = f"{object_type}:{object_id}:{window.starts_at.isoformat()}:{window.ends_at.isoformat()}"
    previous_states = _previous_rule_states(object_type=object_type, object_id=object_id, policy_version=policy.version_key)
    rule_states = dict(previous_states); candidates = []; rule_results = []

    for rule in rules:
        evaluated = evaluate_rule_group(
            signals=facts, rule=rule_spec(rule), parameters=parameters, policy_version=policy.version_key,
            group_identity=group_identity, semantic_window_hours=semantic_window_hours,
        )
        if evaluated is None: continue
        delta, state_after = _temporal_delta(rule=rule, evaluated=evaluated, prior_state=previous_states.get(rule.code))
        ...
        rule_results.append({"rule": rule.code, "channel": rule.channel, "temporal_profile": rule.temporal_profile, "effective_utility": str(delta), "aggregation": rule.aggregation})
    ...
    occurred_at = min(signal.occurred_at for signal in signals); available_at = max(signal.available_at for signal in signals)
    identity = f"{policy.version_key}|object|{object_type}|{object_id}|{window.starts_at.isoformat()}|{window.ends_at.isoformat()}"
    explanation = {
        "object_type": object_type, "object_id": object_id, "rule_results": rule_results, "rule_states": rule_states,
        "utility_delta": str(total_utility), "channels": sorted({row[0].channel for row in selected if row[2] > 0}),
        "signal_ids": [signal.signal_id for signal in signals], "outcomes": [signal.outcome_identity for signal in signals],
    }
    result = process_impact_slice(
        window=window, slice_key="recognition-object:" + hashlib.sha256(identity.encode("utf-8")).hexdigest(),
        accrual_key=f"object:{object_type}:{object_id}:utility", channel=ImpactChannel.UTILITY.value,
        temporal_profile=TemporalProfile.FLOW.value, occurred_at=occurred_at, available_at=available_at,
        impact_delta=total_utility, attribution_shares=[item.share for item in resolved],
        points_target_for_cumulative=_points_target(policy), policy_version=policy.version_key,
        metadata=explanation, maturation_hours=int(parameters.get("maturation_hours", 0) or 0),
    )
    evaluation, _ = RecognitionObjectEvaluation.objects.get_or_create(
        receipt=result.receipt,
        defaults={"window": window, "rule": None, "object_type": object_type, "object_id": object_id, "utility_delta": total_utility, "pool_points": result.receipt.pool_points, "unattributed_points": result.receipt.unattributed_points, "explanation": explanation},
    )
    ...
    for entry in result.grants:
        ...
        RecognitionAllocation.objects.get_or_create(
            evaluation=evaluation, account=entry.account, causal_mode=(detail.causal_mode if detail else CausalMode.OPERATE),
            defaults={"causal_strength": detail.strength if detail else Decimal("1"), "share": detail.share.share if detail else Decimal("0"), "points": entry.points, "evidence": {"signal_ids": explanation["signal_ids"], "outcomes": explanation["outcomes"]}},
        )
        if entry.kind == RecognitionLedgerKind.GRANT.value:
            entry.account.refresh_from_db(); grant_due_achievements(account=entry.account, evaluation=evaluation)
    RecognitionSignal.objects.filter(pk__in=[signal.pk for signal in signals], processed_at__isnull=True).update(processed_at=timezone.now())
...
def run_default_recognition_cycle(*, now=None, max_windows=7):
    ...
    now = now or timezone.now(); released_points = release_due_pending_grants(now=now)
    cursor = RecognitionCursor.objects.filter(pk="recognition-v1").first(); policy = activate_due_policy(now=now, cursor=cursor)
    if policy is None: return {"status": "no_active_policy", "processed_slices": 0, "issued_points": 0, "released_points": released_points}
    ...
    for _ in range(max(int(max_windows), 1)):
        ...
        mark_window_running(window); pending = list(signal_provider(window.starts_at, window.ends_at)); by_object = defaultdict(list)
        for signal in pending: by_object[(signal.object_type, signal.object_id)].append(signal)
        for group in by_object.values(): total_slices += process_signal_group(signals=group, window=window, policy=policy)
        ...
        if next_policy and next_policy.version_key != cursor.policy_version:
            cursor.policy_version = next_policy.version_key; cursor.window_size_hours = next_policy.standard_window_hours; cursor.save(update_fields=["policy_version", "window_size_hours", "updated_at"]); policy = next_policy
    released_points += release_due_pending_grants(now=now)
    return {"status": "ok", "windows": windows, "processed_slices": total_slices, "issued_points": total_points, "released_points": released_points}
