...
class RecognitionCompletionTests(TestCase):
    ...
    def test_signal_contract_drops_private_or_irrelevant_domain_event_fields(self):
        event = SimpleNamespace(
            ...
            payload={
                ...
                "amount": "100.00",
                "currency": "USD",
                "buyer_id": str(self.owner.pk),
                "beneficiary_id": str(self.other.pk),
                "private_note": "must-never-enter-recognition",
            },
        )
    ...
    def test_record_signal_rejects_fields_outside_safe_contract(self):
        with self.assertRaises(ValidationError):
            record_signal(
                ...
                object_type="payment",
                object_id="p-unsafe",
                outcome_identity="payment:p-unsafe:succeeded",
                values={"count": 1, "private_note": "secret"},
            )
    ...
    def test_group_aggregation_is_executed_before_curve(self):
        ...
        rule = RuleSpec(
            ...
            scope={},
            conditions={},
            measure={"op": "field", "name": "count"},
            normalization={"kind": "LINEAR", "factor": 1},
            curve={"kind": "LINEAR", "factor": 2},
            modulators=(),
            aggregation="SUM",
    ...
    def test_internal_correction_never_exposes_negative_balance_and_future_earnings_absorb_it(self):
        ...
        account.points_balance = 5
        account.lifetime_earned = 5
        account.save()
        ...
        self.assertEqual(account.correction_deficit, 5)
        ...
        process_impact_slice(
            window=window,
            slice_key="k:correction:future-earn",
            accrual_key="k:correction:accrual",
            ...
            impact_delta=Decimal("7"),
            attribution_shares=(),
            points_target_for_cumulative=lambda cumulative: int(cumulative),
            policy_version="k:v1",
        )
        # Unattributed pool does not reach the account; create a direct attributed slice.
        process_impact_slice(
            window=window,
            slice_key="k:correction:future-earn-owner",
            accrual_key="k:correction:owner-accrual",
            channel=ImpactChannel.REAL_ACTION.value,
            temporal_profile=TemporalProfile.PULSE.value,
            occurred_at=start + timedelta(hours=2),
            available_at=start + timedelta(hours=2),
            impact_delta=Decimal("7"),
            attribution_shares=(__import__("recognition.services", fromlist=["AttributionShare"]).AttributionShare(profile=self.owner, share=Decimal("1")),),
            points_target_for_cumulative=lambda cumulative: int(cumulative),
            policy_version="k:v1",
        )
        account.refresh_from_db()
        self.assertEqual(account.correction_deficit, 0)
        self.assertEqual(account.points_balance, 2)

    def _feature(self):
        return FeatureDefinition.objects.create(
            code="recognition.k.premium",
            name="Recognition K Premium",
            domain="recognition",
            value_type="boolean",
    ...
    def test_reward_for_other_profile_requires_consent_then_delegates_to_entitlement(self):
        ...
        reward = RewardDefinition.objects.create(
            ...
            version=1,
            name="Premium cadeau",
            kind=RewardKind.ENTITLEMENT,
            points_cost=25,
            beneficiary_allowed=True,
            acceptance_required=True,
            fulfillment={"feature_code": feature.code, "value": True, "duration_days": 30},
        )
        redemption = redeem_reward(
            owner_account=account,
            reward=reward,
            idempotency_key="k-premium-gift-1",
            actor_profile=self.owner,
            beneficiary_profile=self.other,
        )
        account.refresh_from_db()
        self.assertEqual(account.points_balance, 75)
        self.assertEqual(redemption.fulfillment_snapshot["consent_state"], "pending")
        self.assertFalse(EntitlementGrant.objects.filter(profile=self.other, feature=feature).exists())
        with self.assertRaises(ValidationError):
            accept_redemption(redemption=redemption, beneficiary_profile=self.owner)
    ...
    def test_declined_benefit_restores_spent_credits_without_creating_earned_credits(self):
        ...
        reward = RewardDefinition.objects.create(
            code="k-consent-only",
            version=1,
            name="Bénéfice avec consentement",
            points_cost=20,
            beneficiary_allowed=True,
            acceptance_required=True,
        )
