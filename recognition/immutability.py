...
def _validate_reward_contract(instance):
    ...
    for name in ("max_per_owner", "max_per_beneficiary"):
        if name in eligibility:
            try: value = int(eligibility[name])
            except (TypeError, ValueError) as exc: raise ValidationError({"eligibility": f"{name} doit être un entier positif."}) from exc
            if value < 1: raise ValidationError({"eligibility": f"{name} doit être au moins 1."})
    if instance.kind == RewardKind.PAYOUT:
        try: amount = Decimal(str(config.get("amount")))
        except (InvalidOperation, TypeError, ValueError) as exc: raise ValidationError({"fulfillment": "amount doit être un montant positif explicite."}) from exc
...
def protect_published_policy_rule_set(sender, instance, **kwargs):
    if not instance.policy_id: return
...
def protect_achievement_definition(sender, instance, **kwargs):
    if not instance.pk or not AchievementDefinition.objects.filter(pk=instance.pk).exists() or not instance.grants.exists(): return
...
def protect_achievement_delete(sender, instance, **kwargs):
    if instance.pk and instance.grants.exists(): raise ValidationError("Un Achievement déjà accordé est historique et ne peut pas être supprimé.")
...
def protect_reward_definition(sender, instance, **kwargs):
    ...
    if not instance.pk or not RewardDefinition.objects.filter(pk=instance.pk).exists() or not instance.redemptions.exists(): return
...
def protect_reward_delete(sender, instance, **kwargs):
    if instance.pk and instance.redemptions.exists(): raise ValidationError("Une Reward déjà utilisée est historique et ne peut pas être supprimée.")
