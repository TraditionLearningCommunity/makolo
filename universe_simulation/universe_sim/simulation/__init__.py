...
def sr_backend(n=4,beta=.8):
    ...
    momentum=np.zeros((n,3)); momentum[:,0]=gamma*masses*beta*C
    velocity=np.zeros((n,3)); velocity[:,0]=beta*C
    return ArrayStateBackend(tuple(str(i) for i in range(n)),np.zeros((n,3)),velocity,momentum,masses,np.zeros(n),np.zeros(n),np.ones(n,dtype=bool),np.ones(n,dtype=np.int8))


def test_zero_force_is_inertial_and_accumulates_proper_time():
    b=sr_backend(beta=.8); IntegrateurPopulationSRTableau().avancer_forces_constantes(b,np.zeros((4,3)),10)
    assert np.allclose(b.positions_m[:,0],.8*C*10,rtol=1e-14)
    assert np.allclose(b.proper_times_s,6.,rtol=1e-14)


def test_arbitrarily_large_impulse_remains_subluminal():
    b=sr_backend(n=2,beta=0.); force=np.zeros((2,3)); force[:,0]=1e30
    IntegrateurPopulationSRTableau().avancer_forces_constantes(b,force,1e9)
    speeds=np.linalg.norm(b.velocities_m_s,axis=1)
    assert np.all(speeds < C)
    assert np.all(np.isfinite(speeds))


def test_inactive_sr_row_does_not_advance():
    b=sr_backend(n=2,beta=.5); b.active[1]=False; before=b.copier()
    IntegrateurPopulationSRTableau().avancer_forces_constantes(b,np.zeros((2,3)),20)
    assert b.positions_m[0,0] > 0
    assert np.allclose(b.positions_m[1],before.positions_m[1])
    assert b.proper_times_s[1]==before.proper_times_s[1]
...
def test_thousands_of_sr_vehicles_remain_causal_under_distinct_forces():
    ...
    backend = ArrayStateBackend(
        tuple(f"ship-{i}" for i in range(n)),
        np.zeros((n, 3)),
        velocities,
        momenta,
        masses,
