...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        main_org = Organization.objects.create(
            ...
            description="Organisation déterministe pour les parcours navigateur Makolo.",
            city="Lubumbashi",
            country="RDC",
            public_profile=True,
        ...
        finance_org = Organization.objects.create(
            name="Makolo E2E Finance",
        ...
        empty_org = Organization.objects.create(
            ...
            description="Organisation sans événement pour tester les états vides.",
            public_profile=True,
            verification_status=OrganizationVerificationStatus.VERIFIED,
            created_by=users["new_organizer"],
        )
        self._membership(empty_org, users["new_organizer"], OrganizationRole.OWNER)

        sole_org = Organization.objects.create(
            name="Makolo E2E Propriétaire Unique",
            public_profile=False,
            verification_status=OrganizationVerificationStatus.VERIFIED,
            created_by=users["sole_owner"],
        )
        ...
        paid_event = Event.objects.create(
            organizer=users["owner"],
            organization=main_org,
            category=category,
            venue=venue,
            title="Festival Makolo E2E",
            short_description="Le parcours critique de billetterie Makolo.",
            description="Événement public stable destiné aux tests de découverte, paiement et contrôle d’accès.",
            status=EventStatus.PUBLISHED,
            visibility=EventVisibility.PUBLIC,
            start_at=self._dt(2030, 6, 15, 18, 0),
            end_at=self._dt(2030, 6, 15, 23, 0),
            registration_start_at=self._dt(2026, 1, 1, 0, 0),
            registration_end_at=self._dt(2030, 6, 15, 17, 0),
            timezone="Africa/Lubumbashi",
            capacity=200,
            published_at=self._dt(2026, 1, 1, 0, 0),
            metadata={"source": "makolo-e2e"},
        )
        paid_type = TicketType.objects.create(
            event=paid_event,
            name="Pass standard E2E",
            description="Billet payant du scénario de bout en bout.",
            price="12.00",
            currency="USD",
            quantity_total=100,
            min_per_order=1,
        ...
        visual_event = Event.objects.create(
            ...
            category=category,
            venue=venue,
            title="Atelier Makolo Visuel",
            short_description="Événement stable pour les captures de régression visuelle.",
            description="Un événement gratuit avec un billet déterministe pour les snapshots.",
            status=EventStatus.PUBLISHED,
            visibility=EventVisibility.PUBLIC,
            start_at=self._dt(2030, 7, 10, 9, 0),
            end_at=self._dt(2030, 7, 10, 12, 0),
            registration_start_at=self._dt(2026, 1, 1, 0, 0),
            registration_end_at=self._dt(2030, 7, 10, 8, 0),
            timezone="Africa/Lubumbashi",
            capacity=80,
            published_at=self._dt(2026, 1, 1, 0, 0),
            metadata={"source": "makolo-e2e"},
        )
        visual_type = TicketType.objects.create(
            event=visual_event,
            name="Invitation E2E",
            price="0.00",
            currency="USD",
        ...
        create_order(
            buyer=users["visual"],
            event=visual_event,
            customer_name="Visual Participant",
            customer_email=users["visual"].email,
            selections=[(visual_type, 1)],
        )

        gate = EventAccessGate.objects.create(
            event=paid_event,
            name="Entrée E2E",
            description="Porte du scénario QR end-to-end.",
            priority=1,
            created_by=users["owner"],
        )
        ...
        OperationsIncident.objects.create(
            title="Incident démo E2E à ignorer",
            category=IncidentCategory.PAYMENT,
            severity=IncidentSeverity.CRITICAL,
            status=IncidentStatus.OPEN,
            description="Incident marqué démo qui ne doit pas dégrader le health réel.",
            opened_by=users["staff"],
            metadata={"seed": "makolo-demo", "source": "e2e"},
        )
        OperationsIncident.objects.create(
            title="Incident réel E2E visible",
            category=IncidentCategory.ACCESS,
            severity=IncidentSeverity.CRITICAL,
            status=IncidentStatus.OPEN,
            description="Incident opérationnel réel attendu dans le navigateur staff.",
            opened_by=users["staff"],
            event=paid_event,
            organization=main_org,
            metadata={"source": "makolo-e2e"},
    ...
    def _user(self, email, username, **flags):
        user = User.objects.create_user(
            ...
            first_name=username.replace("e2e-", "").replace("-", " ").title(),
            **flags,
        )
