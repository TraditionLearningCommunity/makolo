...
class ParticipantJourneyDetailView(BaseParticipantJourneyDetailView):
    """Personal Journey command center with an explicit participant perspective."""
    ...
    def get_context_data(self, **kwargs):
        ...
        live = (
            ...
            if journey.occurrence
            else None
        )
        context["live"] = live
        context["mature"] = journey_action_presentation(
            journey=journey,
            readiness=context["readiness"],
            live=live,
        )
        context["live_url"] = (
            reverse("core:participant-occurrence-live", kwargs={"pk": journey.occurrence_id})
            if live
            else None
        )
        return context
        ...
        context.update(
            {
                ...
                "access_url": (
                    reverse("core:participant-access-detail", kwargs={"pk": access_id})
                    if access_id
                    else None
                ),
            }
