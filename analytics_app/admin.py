...
class GrowthSpendAdmin(admin.ModelAdmin):
    list_display = (
        "organization", "channel", "label", "amount", "currency", "incurred_at", "created_by",
    )
...
class AnalyticsFactAdmin(admin.ModelAdmin):
    list_display = (
        "fact_type", "space", "activity", "occurrence", "numeric_value", "currency", "occurred_at",
    )
    ...
    readonly_fields = (
        "domain_event", "fact_type", "space", "activity", "occurrence", "profile",
        "numeric_value", "currency", "occurred_at", "created_at",
    )
