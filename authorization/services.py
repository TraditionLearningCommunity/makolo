...
def _mandates_with_permissions(profile, *, space=None, group=None, activity=None, dossier=None, at=None):
    queryset = (
        Mandate.objects.filter(profile=profile)
        .filter(_current_mandate_q(at))
        .filter(role__is_active=True)
        .select_related("role", "space", "group", "activity", "dossier")
        .prefetch_related(
            Prefetch(
    ...
    if dossier is not None:
        queryset = queryset.filter(Q(scope_type=AuthorityScope.PLATFORM) | Q(scope_type=AuthorityScope.DOSSIER, dossier=dossier))
...
def can_many(profile, permission_codes, space=None, *, group=None, activity=None, dossier=None, at=None) -> dict[str, bool]:
    requested = tuple(dict.fromkeys(permission_codes))
    if activity is not None or dossier is not None:
        return {code: can(profile, code, space, group=group, activity=activity, dossier=dossier, at=at) for code in requested}
...
def space_ids_with_permission(profile, permission_code: str, *, at=None):
    ...
    return list(
        Mandate.objects.filter(
            profile=profile, scope_type=AuthorityScope.SPACE, role__is_active=True,
...
def group_ids_with_permission(profile, permission_code: str, *, at=None):
    ...
    return list(
        Mandate.objects.filter(
            profile=profile, scope_type=AuthorityScope.GROUP, role__is_active=True,
            role__role_permissions__permission__code=permission_code,
            role__role_permissions__permission__is_active=True,
...
def activity_ids_with_permission(profile, permission_code: str, *, at=None):
    ...
    direct = set(
        Mandate.objects.filter(
            profile=profile, scope_type=AuthorityScope.ACTIVITY, role__is_active=True,
            role__role_permissions__permission__code=permission_code,
            role__role_permissions__permission__is_active=True,
...
def dossier_ids_with_permission(profile, permission_code: str, *, at=None):
    if not _authenticated(profile):
        return []
    if getattr(profile, "is_superuser", False) or has_platform_authority(profile, at=at):
        return None
    return list(
        Mandate.objects.filter(
            profile=profile, scope_type=AuthorityScope.DOSSIER, role__is_active=True,
            role__role_permissions__permission__code=permission_code,
            role__role_permissions__permission__is_active=True,
        ).filter(_current_mandate_q(at)).exclude(dossier_id=None).values_list("dossier_id", flat=True).distinct()
    )
...
def validate_role_for_dossier(role: Role) -> None:
    if not role.is_active or role.scope_type != AuthorityScope.DOSSIER or not role.is_system:
        raise ValidationError("Ce rôle ne peut pas être accordé sur un Dossier.")
    if role.organization_id is not None or role.code not in STANDARD_DOSSIER_ROLE_CODES:
        raise ValidationError("Ce rôle Dossier n'est pas un rôle système pris en charge.")
...
def grant_space_role(*, profile, space, role, granted_by=None, source="service") -> Mandate:
    ...
    mandate.full_clean(); mandate.save(); return mandate
...
@transaction.atomic
def grant_activity_role(*, profile, activity, role=None, role_code=None, granted_by=None, source="activity-service") -> Mandate:
...
@transaction.atomic
def grant_dossier_role(*, profile, dossier, role, granted_by=None, source="objectives-service", valid_from=None, valid_until=None) -> Mandate:
    if isinstance(role, str):
        role = get_system_role(role, scope_type=AuthorityScope.DOSSIER)
    validate_role_for_dossier(role)
    existing = Mandate.objects.select_for_update().filter(profile=profile, role=role, scope_type=AuthorityScope.DOSSIER, dossier=dossier, status=MandateStatus.ACTIVE).order_by().first()
    if existing:
        return existing
    mandate = Mandate(profile=profile, role=role, scope_type=AuthorityScope.DOSSIER, dossier=dossier, status=MandateStatus.ACTIVE, valid_from=valid_from, valid_until=valid_until, granted_by=granted_by, source=source)
    mandate.full_clean(); mandate.save(); return mandate
...
def replace_standard_space_role(*, profile, space, role_code: str, granted_by=None, source="team-service") -> Mandate:
    ...
    for mandate in current:
        ...
        mandate.status = MandateStatus.REVOKED; mandate.revoked_at = timezone.now(); mandate.save(update_fields=["status", "revoked_at", "updated_at"])
...
def revoke_all_space_mandates(*, profile, space, actor=None) -> int:
    ...
    now = timezone.now()
    for mandate in mandates:
        mandate.status = MandateStatus.REVOKED; mandate.revoked_at = now; mandate.save(update_fields=["status", "revoked_at", "updated_at"])
    return len(mandates)
...
def require(profile, permission_code: str, space=None, *, group=None, activity=None, dossier=None, message=None) -> None:
    if not can(profile, permission_code, space, group=group, activity=activity, dossier=dossier):
