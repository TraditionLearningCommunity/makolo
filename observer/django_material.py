...
def build_observation_material(
    ...
    attempts = tuple(
        ...
        for attempt in observation.attempts.all().order_by(
            "ordinal",
            "id",
        )
    )
    ...
    for artifact in observation.artifacts.all().order_by(
        ...
        descriptors.append(descriptor_for(artifact))
    ...
    revalidated_descriptors = tuple(
        descriptor_for(artifact)
        for artifact in revalidated
    )
    revalidated_refs = tuple(
        descriptor.artifact_ref
        for descriptor in revalidated_descriptors
    )
