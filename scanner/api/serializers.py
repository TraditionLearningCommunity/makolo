...
class ScannerAssignmentSerializer(serializers.ModelSerializer):
    ...
    agent_id = serializers.PrimaryKeyRelatedField(
        source="agent", queryset=User.objects.filter(is_active=True), write_only=True,
    )
    ...
    class Meta:
        ...
        fields = [
            "id", "event", "event_id", "activity_id", "occurrence_id", "checkpoint_id",
            "agent", "agent_id", "access_gate", "access_gate_id", "assigned_by", "label",
            "is_active", "is_current", "valid_from", "valid_until", "notes", "created_at", "updated_at",
        ]
    ...
    def validate(self, attrs):
        ...
        if checkpoint is not None:
            ...
            if occurrence is not None and occurrence.pk != checkpoint_occurrence.pk:
                raise serializers.ValidationError({"checkpoint_id": "Ce checkpoint appartient à une autre Occurrence."})
            if activity is not None and activity.pk != checkpoint_occurrence.activity_id:
                raise serializers.ValidationError({"checkpoint_id": "Ce checkpoint appartient à une autre Activity."})
        ...
        if event is not None:
            ...
            if occurrence is None:
                ...
                raise serializers.ValidationError({"occurrence_id": "Cette Occurrence ne correspond pas à l’Event."})
        ...
        if access_gate and (event is None or access_gate.event_id != event.pk):
            raise serializers.ValidationError({"access_gate_id": "Cette porte appartient à un autre événement."})

        # Assignment records responsibility only. The target Profile must
        # already hold canonical scan authority; creating the Assignment never
        # creates or substitutes a Mandate.
        if agent is not None and not user_can_scan_activity(agent, activity, occurrence=occurrence):
            raise serializers.ValidationError(
                {"agent_id": "Accordez d’abord au Profil une Permission/Mandat de contrôle pour cette Activity."}
            )
        ...
        if agent is not None:
            ...
            duplicates = duplicates.filter(occurrence=occurrence) if occurrence is not None else duplicates.filter(occurrence__isnull=True)
            if duplicates.exists():
                field = "occurrence_id" if occurrence is not None else "activity_id"
                raise serializers.ValidationError({field: "Cet agent possède déjà une affectation dans ce scope Scanner."})
