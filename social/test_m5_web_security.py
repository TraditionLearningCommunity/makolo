...
class M5WebSecurityTests(TestCase):
    def setUp(self):
        self.owner = User.objects.create_user(
            username="m5-web-owner",
            email="m5-web-owner@example.test",
            password="StrongPass2026!",
        )
        self.member = User.objects.create_user(
            username="m5-web-member",
            email="m5-web-member@example.test",
            password="StrongPass2026!",
        )
        self.outsider = User.objects.create_user(
            username="m5-web-outsider",
            email="m5-web-outsider@example.test",
            password="StrongPass2026!",
        )
    ...
    def test_outsider_cannot_reply_to_private_group_contribution(self):
        ...
        response = self.client.post(
            reverse("social:reply", kwargs={"pk": self.contribution.pk}),
            {"body": "Intrusion"},
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_owner_cannot_change_another_profiles_goal_through_web(self):
        ...
        self.client.force_login(self.outsider)
        response = self.client.post(
            reverse("goals:status", kwargs={"pk": goal.pk}),
            {"status": "paused"},
        )
        self.assertEqual(response.status_code, 403)
