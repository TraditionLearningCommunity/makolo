...
def build_action_stream(profile, *, offset=0, limit=20, at=None, context=None):
    ...
    def merge(item):
        ...
        items[item.key] = replace(
            existing,
            reasons=reasons,
            occurred_at=max(existing.occurred_at, item.occurred_at),
        )

    followed_ids = list(
        OrganizationFollow.objects.filter(user=profile)
        .values_list("organization_id", flat=True)[:STREAM_SOURCE_LIMIT]
    )
    if followed_ids:
        activities = (
        ...
        updates = (
            ...
            .select_related("author_profile", "space", "activity", "occurrence")
            .order_by("-created_at", "id")[:STREAM_SOURCE_LIMIT]
        )
        for contribution in updates:
            ...
            merge(ActionStreamItem(
                ...
                title=contribution.activity.title if contribution.activity_id else contribution.space.name,
                summary=contribution.body,
                activity=contribution.activity,
                contribution=contribution,
                reasons=("Mise à jour officielle d'un Espace suivi",),
                cta_label=cta_label,
                cta_url=cta_url,
            ))
    ...
    contributions = (
        ...
        .select_related("author_profile", "space", "group", "activity", "occurrence")
        .order_by("-created_at", "id")[:STREAM_SOURCE_LIMIT]
    )
    for contribution in contributions:
        ...
        if contribution.activity_id:
            cta_label, cta_url = activity_destination(contribution.activity)
            key = (
                f"activity:{contribution.activity_id}"
                if contribution.kind == ContributionKind.SHARE
                else f"contribution:{contribution.pk}"
            )
        else:
            cta_label, cta_url = "", ""
            key = f"contribution:{contribution.pk}"
    ...
    for recommendation in build_activity_recommendations(
        ...
        merge(ActionStreamItem(
            ...
            kind="recommendation",
            occurred_at=recommendation.activity.updated_at,
