...
from .services import ShareUnavailable, _create_delivery, _create_envelope, _validate_direct_participants
...
def _require_shareable_source(journey, actor):
    ...
    if template is None or template.status not in {
        ServicePlanTemplateStatus.PUBLISHED,
        ServicePlanTemplateStatus.RETIRED,
    }:
...
def build_journey_share_snapshot(*, journey, actor):
    ...
    for step in template_steps:
        steps.append(
            {
                "ref": step_refs[step.pk],
                "template_step_id": str(step.pk),
                "kind": step.kind,
                "title": step.title,
                "description": step.description,
                "position": step.position,
                "is_required": step.is_required,
                "classification": _step_classification(step.kind),
                "depends_on": [
                    step_refs[dependency.depends_on_id]
                    for dependency in step.dependencies.all()
                    if dependency.depends_on_id in step_refs
                ],
            }
        )

    form_requests = list(
        journey.form_requests.select_related("form_version", "form_version__form")
        .order_by("created_at", "id")[: MAX_FORMS + 1]
    )
    if len(form_requests) > MAX_FORMS:
        raise ValidationError("Ce parcours référence trop de formulaires pour être partagé.")
    forms = [
        {
            "form_id": str(request.form_version.form_id),
            "source_form_version_id": str(request.form_version_id),
            "source_version": request.form_version.version,
            "required": request.required,
            "classification": PERSONALIZE,
        }
    ...
    if context.opportunity_id and context.opportunity_revision_id:
        opportunity = {
            ...
            "source_revision_id": str(context.opportunity_revision_id),
            "source_version": context.opportunity_revision.version,
    ...
    counts = _snapshot_counts(all_items)
    snapshot = {
        "schema_version": SNAPSHOT_SCHEMA_VERSION,
        "workflow": journey.workflow,
        "activity_id": str(journey.activity_id),
        "occurrence_id": str(journey.occurrence_id) if journey.occurrence_id else None,
        "service_template": {
...
def create_direct_journey_share(*, created_by, recipient, journey, expires_at=None):
    ...
    envelope = _create_envelope(
        ...
        subject_type=ShareSubjectType.JOURNEY,
        intent=ShareIntent.START_JOURNEY,
        expires_at=expires_at,
    )
    subject = JourneyShareSubject.objects.create(
        envelope=envelope,
...
def _current_template(subject, source_journey):
    ...
    try:
        ...
        raise ShareUnavailable from exc
    template = (
        ServicePlanTemplate.objects.filter(
            service=service,
            key=key,
            status=ServicePlanTemplateStatus.PUBLISHED,
        )
...
def evaluate_journey_share(subject):
    ...
    if opportunity_snapshot:
        try:
            source_context = source_journey.service_context
            opportunity = source_context.opportunity
        except Exception as exc:
            raise ShareUnavailable from exc
    ...
    for form_snapshot in subject.snapshot.get("forms", []):
        ...
        if form is not None:
            current_version = (
                form.versions.filter(status=FormVersionStatus.PUBLISHED).order_by("-version", "-created_at").first()
            )
    ...
    for resource_snapshot in subject.snapshot.get("resources", []):
        query = ActivityResource.objects.filter(
            activity_id=source_journey.activity_id,
            key=resource_snapshot.get("key"),
            status=ResourceStatus.PUBLISHED,
            visibility=ResourceVisibility.PUBLIC,
        )
    ...
    counts.setdefault(REUSABLE, 0)
    counts.setdefault(PERSONALIZE, 0)
    counts.setdefault(REVALIDATE, 0)
    counts[REVALIDATE] += len(stale) + form_revalidation + resource_revalidation
    return {
        "counts": counts,
        "stale": stale,
        "form_revalidation": form_revalidation,
        "resource_revalidation": resource_revalidation,
...
def accept_journey_share(*, delivery_id, user):
    ...
    delivery = (
        ShareDelivery.objects.select_for_update(of=("self",))
        .select_related("envelope", "recipient", "recipient__user")
        .get(pk=delivery_id)
    )
    ...
    subject = JourneyShareSubject.objects.select_related(
        "source_journey",
        "source_journey__activity",
        "source_journey__occurrence",
        "source_journey__activity__service_details",
    ).get(envelope=delivery.envelope)
    evaluation = evaluate_journey_share(subject)
    source_journey = subject.source_journey
    journey = create_reused_service_journey(
        source_journey=source_journey,
        recipient=user,
        template=evaluation["current_template"],
        opportunity=evaluation["opportunity"],
        opportunity_revision=evaluation["opportunity_revision"],
    )
