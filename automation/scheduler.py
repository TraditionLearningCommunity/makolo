...
def run_autopilot_cycle(*, now=None, delivery_limit=100):
    ...
    stats["spatiotemporal"] = run_spatiotemporal_automation_cycle(now=now, limit=max(delivery_limit, 1))
    stats["proactive_preparation"] = run_proactive_preparation_cycle(now=now, limit=max(delivery_limit, 1))
